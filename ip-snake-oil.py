...
# Add the four binary octets from the binary_octets list into one string 'binary_whole_ip',
# ensuring each one is padded with zeros to make it 8 chars in length
for octet in binary_octets:
    binary_whole_ip += str(octet).zfill(8)

# Now the ip address is stored as 4 integers in the octets list, as 4 binary numbers in the
# binary_octets list, and as a whole binary string in the binary_whole_ip variable:
...
subnet_mask = (f'{int(mask_bits[0:8], 2)}.{int(mask_bits[8:16], 2)}.'
               f'{int(mask_bits[16:24], 2)}.{int(mask_bits[24:32], 2)}')
...
network_ip = (f'{int(network_binary[0:8], 2)}.{int(network_binary[8:16], 2)}.'
              f'{int(network_binary[16:24], 2)}.{int(network_binary[24:32], 2)}')
...
# Convert to dotted decimal notation:
broadcast_ip = (f'{int(broadcast_binary[0:8], 2)}.{int(broadcast_binary[8:16], 2)}.'
                f'{int(broadcast_binary[16:24], 2)}.{int(broadcast_binary[24:32], 2)}')
